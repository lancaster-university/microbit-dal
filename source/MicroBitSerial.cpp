...
int MicroBitSerial::initialiseRx()
{
    ...
    memset((void *)this->rxBuff, 0, rxBuffSize);
...
int MicroBitSerial::initialiseTx()
{
    ...
    memset((void *)this->txBuff, 0, txBuffSize);
...
/**
  ...
  *
  * @return the a character from the circular buffer.
  */
char MicroBitSerial::getChar(MicroBitSerialMode mode)
{
    if(mode == ASYNC)
    {
        if(!isReadable())
...
/**
  ...
  */
char MicroBitSerial::read(MicroBitSerialMode mode)
{
    if(rxInUse())
        return 0;
...
#if CONFIG_ENABLED(MICROBIT_DBG)
    uBit.serial.printf("read returned: %d\r\n", returnedSize);
#endif
...
int MicroBitSerial::read(uint8_t *buffer, int bufferLen, MicroBitSerialMode mode)
{
    ...
    if(mode == ASYNC)
    {
        while(isReadable() && bufferIndex < bufferLen)
        {
            buffer[bufferIndex] = getChar(mode);
            bufferIndex++;
        }
    }
...
/**
  ...
  * @return MICROBIT_INVALID_PARAMETER if the mode given is SYNC_SPINWAIT, otherwise MICROBIT_OK.
  */
int MicroBitSerial::eventOn(ManagedString delims, MicroBitSerialMode mode)
{
    if(mode == SYNC_SPINWAIT)
        return MICROBIT_INVALID_PARAMETER;
